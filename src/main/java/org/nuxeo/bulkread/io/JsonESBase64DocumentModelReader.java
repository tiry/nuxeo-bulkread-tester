 ...
 * @since 5.9.5
 */
public class JsonESBase64DocumentModelReader {
    private static final Log log = LogFactory.getLog(JsonESBase64DocumentModelReader.class);

    private final Map<String, Object> source;

    private String sid;

    public JsonESBase64DocumentModelReader(String json) {
        byte[] bytes = json.getBytes();
        source = SourceLookup.sourceAsMap(new BytesArray(bytes, 0, bytes.length));
    }

    public JsonESBase64DocumentModelReader(Map<String, Object> source) {
        this.source = source;
    }

    public JsonESBase64DocumentModelReader session(CoreSession session) {
        sid = session.getSessionId();
        return this;
    }

    public JsonESBase64DocumentModelReader sid(String sid) {
        this.sid = sid;
        return this;
    }
    ...
    public DocumentModel getDocumentModel() {
        ...
        DocumentModelImpl doc = new DocumentModelImpl(sid, type, id, pathObj, docRef, parentRef, null, facets,
                sourceId, repositoryName, isProxy);
        ...
        for (String prop : source.keySet()) {
            ...
            // regular property
            try {
                if (prop.equals("content")) { // XXX do better !

                    Map<String, Serializable> blobProp = (Map<String, Serializable>) value;
                    String mt = (String) blobProp.get("mime-type");
                    String enc = (String) blobProp.get("encoding");
                    String fname = (String) blobProp.get("name");

                    if (blobProp.containsKey("base64")) {
                        // build from base64 stored in ES
                        byte[] data = Base64.decodeBase64((String) blobProp.get("base64"));
                        ByteArrayBlob blob = new ByteArrayBlob(data, mt, enc);
                        blob.setFilename(fname);
                        doc.setPropertyValue(prop, blob);
                    } else {
                        // fetch from BinaryManager
                        String digest = (String) blobProp.get("digest");
                        BlobManager bm = Framework.getService(BlobManager.class);
                        BlobProvider bp = bm.getBlobProvider(doc.getRepositoryName());
                        Binary bin = bp.getBinaryManager().getBinary(digest);
                        BinaryBlob blob = new BinaryBlob(bin, digest, fname, mt, enc, digest, bin.getFile().length());
                        doc.setPropertyValue(prop, blob);
                    }
